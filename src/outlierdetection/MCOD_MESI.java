...
//    public static HashMap<Cluster, HashSet<MCData>> associates = new HashMap<>();
    ...
    public static double p_add_to_cluster = 0;
...
//    public static ArrayList<Integer> number_points_in_clusters = new ArrayList<>();
    ...
    public ArrayList<Data> detectOutlier(ArrayList<Data> data, int _currentTime, int W, int slide) {
        ...
        if (data.size() == slide) {
            int expiredSlideIndex = (currentTime  - 1) /slide - Constants.W /slide;
...
//            assert newestSlideIdx == d.getSlideIndex();
        ...
        if (data.size() > 0) {
            ...
            p_add_to_cluster = (p_add_to_cluster * countWindow + added_to_cluster * 1.0 / data.size())/(countWindow +1);
            countWindow +=1;
        }
        //find outlier     
...
//        for(Integer sIdx: PDList.keySet()){
//            System.out.println("PD key "+sIdx +":  "+ PDList.get(sIdx).size());
//        }
//        for(Cluster c: microClusters){
//            System.out.println("Micro Cluster !!!!");
//            for(Integer sIdx: c.slide_members.keySet()){
//                System.out.println("Cluster key "+sIdx +" :"+ c.slide_members.get(sIdx).size());
//            }
//        }
...
//                System.out.println("Not enough members or expired!!!!!!!!!!");
        ...
        System.out.println("DispersedList size = "+ dispersedList.size());
        //process dispersed list
        dispersedList.stream().forEach((d) -> {
            PDList.get(d.getSlideIndex()).add(d);
        });
        dispersedList.stream().forEach((d) -> {
            processData(d, true);
        });
...
//            }
    ...
    public void probe(MCData d, boolean isFromDispersedList) {
        ...
        while ((slideIndex > d.getSlideIndex() - Constants.W / Constants.slide)
                && (closeNeighbors.size() < Constants.k)
                && slideIndex >= 0) {
