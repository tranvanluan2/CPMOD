                ...
                for (C_Data d : all_slides.get(sIdx)) {
                    ...
                    for (OD_Query q : all_queries) {
                        ...
                        if (currentTime - q.W < d.arrivalTime
                            ...
                            if (!d.safeInlierQueries.contains(q)
                                    && (!d.closeCoreMaps_halfR.containsKey(q.R)
                        ...
                        for (int idx = newestSlide; idx > expiredSlideIndex; idx--) {
                            ...
                            if (total_neighbor >= k_max) {
                                if (total_suc_neighbor >= k_max) {
                                    for (Integer w : all_r_w.get(r).keySet()) {
//                                        if (idx >= newestSlide - all_w.get(0) / Constants.slide + 1) {
                                        d.safeInlierQueries.addAll(all_r_w.get(r).get(w));
//                                        }
                                    }
                                    //queries with larger R are also safe
                                }
                                
                                //queries with larger R and larger W are also inliers  
                                break;
                            } else {
//                                if (idx == newestSlide - all_w.get(0) / Constants.slide + 1) {
                                while (!all_w.isEmpty()
                                        && all_slides.get(idx).get(0).arrivalTime == currentTime + 1 - all_w.get(0)) {
...
//    }
    private ResultFindCore findCloseCore(C_Data d, int slideIndex, double radius) {
        ...
        if (d.closeCoreMaps_halfR.get(radius) != null
            ...
            if (all_core_points_map.get(slideIndex) == null) {
                System.out.println("slideIndex = " + slideIndex);
                System.out.println("Key = ");
                for (Integer idx : all_core_points_map.keySet()) {
...
//        public HashMap<Double, Integer> lastHalfR = new HashMap<>();
        ...
        public void discountHalfPoints(Double r) {
            if (totalHalfRPoints.containsKey(r)) {
                ...
                for (Integer w : totalHalfRPoints.get(r).keySet()) {
                    int expiredSlide = -1;
                    for (Integer sIdx : current_slides) {
                        if (all_slides.get(sIdx).get(0).arrivalTime <= currentTime + 1 - w) {
                            expiredSlide = sIdx;
                            int t = 0;
                            for (Bin b : all_bins) {
                                if (b.max_val <= r / 2) {
        ...
        public void computeTotalHalfRPoints(Double r) {
            ArrayList<Integer> all_w = new ArrayList<>(all_r_w.get(r).keySet());
...
//                System.out.println("other slide = "+(newestSlide - all_w.get(0) / Constants.slide + 1));
                while (!all_w.isEmpty() && all_slides.get(idx).get(0).arrivalTime == currentTime + 1 - all_w.get(0) //                        idx == newestSlide - all_w.get(0) / Constants.slide + 1
                        ) {
...
//        }
    ...
    class C_Data extends Data {
        ...
        private void reset() {
            ...
            this.closeCoreMaps_R.clear();
            this.closeCoreMaps_halfR.clear();
