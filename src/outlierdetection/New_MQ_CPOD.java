 ...
 * @author luan
 */
public class New_MQ_CPOD {

    public static int currentTime;

    public static int expiredSlideIndex = -1;
    public static ArrayList<Integer> all_slide_idx = new ArrayList<>();
...
//    public static double min_r = Double.MAX_VALUE;
    ...
    public static int last_sidx = 0;

    public static ArrayList<Data> buffer = new ArrayList<>();

    public ArrayList<Integer> getSlides(int currentTime, int w_start) {
    ...
    public HashMap<OD_Query, ArrayList<Data>> slide_process(ArrayList<Data> data, int _currentTime) {
        ArrayList<OD_Query> active_queries = get_active_queries(all_queries, _currentTime);
        if (active_queries.isEmpty()) {
            buffer.addAll(data);
            return new HashMap<OD_Query, ArrayList<Data>>();
        } else {

            HashMap<OD_Query, ArrayList<Data>> result = new HashMap<>();
            for (OD_Query q : active_queries) {
            ...
            if (currentTime == Constants.W) { //max W
                ArrayList<Integer> slideOffsets = getSlides(currentTime, 0);

                slide_to_process = new int[slideOffsets.size()];
                for (int i = 0; i < slide_to_process.length; i++) {
                    slide_to_process[i] = i;
                    all_slide_idx.add(i);
                }
                last_sidx = slide_to_process.length - 1;
                int cur_sidx = 0;
                for (int i = 0; i < data.size(); i++) {
                    Data o = data.get(i);
                    if (i == slideOffsets.get(cur_sidx + 1)) {
                        cur_sidx += 1;
                    }
                ...
                last_sidx += 1;
                slide_to_process = new int[]{last_sidx};
                all_slide_idx.add(last_sidx);
                for (int i = 0; i < data.size(); i++) {
                    Data o = data.get(i);
                    C_Data d = new C_Data(o);
                    d.sIndex = last_sidx;
...
//            }
            ...
            if(currentTime > Constants.W){
                ...
                expiredSlideIndex = all_slide_idx.get(0);
            }
...
//        System.out.println("End testing ====================");
    ...
    public void add_query(OD_Query q) {
        ...
        Collections.sort(all_r_queries);
...
//        Collections.sort(all_r_queries);
        if (d.lastProbRight < newestSlide) {
            ...
            while (slideIndex <= newestSlide && !queries.isEmpty()) {
                ...
                if (all_core_points_map.get(slideIndex).get(max_r).isEmpty()) {
                    //find larger r with that we have supporting core points
                    for (Double r : all_r_queries) {
                        if (r > max_r && !all_core_points_map.get(slideIndex).get(r).isEmpty()) {
                            max_r = r;
                ...
                probe_slide_right(d, slideIndex, queries, max_r);
        ...
        while (slideIndex > expiredSlideIndex && slideIndex >= 0 && !queries.isEmpty()) {
            ...
            //probe left
            probe_slide_left(d, slideIndex, queries, max_r);
                    ...
                    if (case_ == 0 || (case_ == 1 && !checked[d2.arrivalTime - min_arrival_time])) {
                        ...
                        for (Double r : all_r_in_queries) {
                            if (dist <= r) {
                                ...
                                //check to remove inlier query
                                for (OD_Query q : r_query_map.get(r)) {
                                    if (q.k <= newNeighborCount.get(r)) {
                                        queries.remove(q);
                                    }
                                }
                            }
    ...
    private ArrayList<OD_Query> get_active_queries(ArrayList<OD_Query> all_queries, int _currentTime) {
        ArrayList<OD_Query> active_queries = new ArrayList<>();
        for (OD_Query q : all_queries) {
            if ((_currentTime - q.W) / q.S * q.S == (_currentTime - q.W)) {
...
//        for (Double radius : all_r) {
            ...
            if (lastgamma > 0) {
                ...
                ArrayList<Double> threshold = get_threshold_new(data.size(), data.size(), lastalpha, lastbeta, lastgamma);
