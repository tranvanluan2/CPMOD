...
//            double distance = mtree.getDistanceFunction().calculate(o, inPD);
            double distance = check_distance_neighbor(o, inPD);
...
//        mtree.add(o);
    ...
    public double check_distance_neighbor(MCO d, MCO d2) {
    ...
    public double check_distance_3R2_neighbor(MCO d, MCO d2) {
        double lb_d = DistanceFunction.LB_distance2(d, d2);
    ...
    public double check_distance_R2_neighbor(MCO d, MCO d2) {
        double lb_d = DistanceFunction.LB_distance2(d, d2);
...
//            double distance = mtree.getDistanceFunction().calculate(center, d);
            double distance = check_distance_R2_neighbor(center, d);
    ...
    public ArrayList<Integer> findClusterIn3_2Range(MCO d) {
        ...
        micro_clusters.keySet().stream().forEach((center_id) -> {
            ...
            //compute the distance
//            double distance = mtree.getDistanceFunction().calculate(center, d);
            double distance = check_distance_3R2_neighbor(center, d);
    ...
    private void processNewData(Data data) {
        ...
        time_adding_to_cluster += Utils.getCPUTime() - startTime3;
...
//            timeForAddToCluster += Utils.getCPUTime() - startTime3;
            ...
            if (neighborsInR2Distance.size() >= Constants.k + 1) {
                //form new cluster
                startTime3 = Utils.getCPUTime();
...
//                timeForAddToPD += Utils.getCPUTime() - startTime3;
            ...
            time_finding_neighbors += Utils.getCPUTime() - startTime3;
        }
        if (d.isCenter) {
...
//                    addToHashMap(inPD.arrivalTime,d.arrivalTime);
    ...
    public ArrayList<MCO> findNeighborR2InPD(MCO d) {
        ...
        PD.stream().filter((o) -> ( //                mtree.getDistanceFunction().calculate(o, d) <= Constants.R * 1.0 / 2
                check_distance_R2_neighbor(o, d) <= Constants.R / 2)
        ).forEach((o) -> {
            results.add(o);
        });
        return results;
...
//
//        PD.stream().filter((o) -> (
//                mtree.getDistanceFunction().calculate(o, d) <= Constants.R
//                chec
//                )).forEach((o) -> {
//            result.add(o);
//        });
...
//            double distance = mtree.getDistanceFunction().calculate(p, d);
            double distance = check_distance_3R2_neighbor(p, d);
            if (distance <= Constants.R * 3.0 / 2) {
                result.add(p);
            }
        });
...
//        double d = mtree.getDistanceFunction().calculate(p, o);
        double d = check_distance_neighbor(p, o);
