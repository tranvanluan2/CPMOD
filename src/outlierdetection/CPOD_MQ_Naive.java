    ...
    public HashMap<OD_Query, HashSet<Data>> slide_process(ArrayList<Data> data, int _currentTime) {
        ...
        for (OD_Query q : all_queries) {
            for (int i = 0; i < d_to_process.size(); i++) {
                ...
                if (d.closeCoreMaps_halfR.get(q) != null && d.closeCoreMaps_halfR.get(q).totalHalfRPoints >= q.k + 1) {
                    continue;
                }
...
//            System.out.println("Finished "+ i);
            ...
            if (outlierList_map.get(q) != null) {
                for (Map.Entry<Integer, HashSet<C_Data>> e : outlierList_map.get(q).entrySet()) {
//        for (int slideIndex : outlierList.keySet()) {
                    for (C_Data d : e.getValue()) {
                        ...
                        if (d.neighborCount.get(q) < q.k && d.sIndex < newestSlide) {
                            if (d.lastProbRight.get(q) < newestSlide) {
