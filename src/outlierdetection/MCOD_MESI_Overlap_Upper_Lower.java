 ...
 * @author Luan Tran
 */
public class MCOD_MESI_Overlap_Upper_Lower {
...
//                            distance = DistanceFunction.euclideanDistance(d, d2);
        ...
        if (d.clusters.isEmpty()) {
            ...
            int num_closeNeighborInPD = closeNeighbors.size();

            slideIndex = d.getSlideIndex();
            int countNeighbor = 0;
            boolean canFormCluster = false;
            ...
            while ((slideIndex > d.getSlideIndex() - Constants.W / Constants.slide)
                        ...
                        if (dataList != null) {
                            for (MCData d2 : dataList) {
                                if (d2.arrivalTime != d.arrivalTime) {
                                    ...
                                    if (distance <= Constants.R) {
                                        neighbors.add(d2);
                                        if (num_closeNeighborInPD > Constants.minSizeOfCluster * 0.6) {
                                            if (distance <= Constants.R / 2 && d2.clusters.size() < 2) {
                                                closeNeighbors.add(d2);
                                            }
                ...
                if (d.clusters.isEmpty() && neighbors != null) {
    ...
    private void formNewCluster(MCData d, ArrayList<MCData> closeNeighbors) {
        ...
        for (MCData d2 : closeNeighbors) {
            ...
            if (d2.clusters.isEmpty() && d2.isUnsafeInlier()) {
                event_queue.remove(d2);
            }
            if (d2.clusters.isEmpty()) {
